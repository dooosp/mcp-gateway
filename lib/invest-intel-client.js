...
// ─── Graham Gate / Value Snapshot API ──────────────────────────
...
/**
 * Graham Gate 운영 준비 상태 조회
 * GET /api/value/readiness
 */
export async function getGrahamReadiness() {
  return fetchWithRetry('/api/value/readiness');
}

/**
 * valueSnapshot 감사 기록 조회
 * GET /api/value/snapshots
 */
export async function getValueSnapshots({ stockCode, decision, source, limit = 20 } = {}) {
  return fetchWithRetry(`/api/value/snapshots${toQuery({ stockCode, decision, source, limit })}`);
}

/**
 * Graham Gate 기준별 pass/fail 및 거절 사유 요약
 * GET /api/value/snapshots/summary
 */
export async function getValueSnapshotSummary({ stockCode, days } = {}) {
  return fetchWithRetry(`/api/value/snapshots/summary${toQuery({ stockCode, days })}`);
}

/**
 * valueSnapshot export payload 조회
 * GET /api/value/snapshots/export
 */
export async function exportValueSnapshots({ stockCode, decision, source, limit = 500 } = {}) {
  return fetchWithRetry(`/api/value/snapshots/export${toQuery({ stockCode, decision, source, limit })}`);
}
